...
fun takeFile (x :string) = let val mylexer = fileToLexer (x) in (
			...
			isParseError := false;
			myData.parsedList := [];
			let val r = parse (mylexer) in  (
			if ((!isParseError) = false ) then (	print ("\n\nLEXER OUTPUT: \n\n");
								printStringList (CalcLex.UserDeclarations.lexedList); 
								print ("\n\n PARSER OUTPUT: \n\n");
								printParsedList (myData.parsedList);
								print ("\n\n EVALUATED ABSTRACT SYNTAX TREE: \n\n");
								print (!(PT.evaluationOutput));
								print ("\n\n")
								) else ();
			isParseError := false; r ) end
	...
	| PT.brokenTypes (s :string) => print(s)
	| PT.typeIncorrect (s :string) => print(s)
	| PT.notABool (s :string) => print(s)
	| PT.scopeError (s :string) => print(s)
	| PT.functionEvaluationError (s :string) => print(s)
	| ParseError => ();
